...
CUSTOM_TRIGGERS = {
    ...
    "hi": [  # Lowercase ထည့်ပေးခြင်းဖြင့် အလွယ်တကူ မိစေရန်
        "Hello",
        "စကားဝင်ပြောအသဲ",
    ],
    "နယူး": [
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ၂။ Bot ကို Reply လုပ်ခြင်း (သို့) နာမည်ခေါ်ခြင်း စနစ် (Auto Reply)
    ...
    if chat_type in ["group", "supergroup"]:
        if message.reply_to_message and message.reply_to_message.from_user.id == context.bot.id:
            is_mentioned = True
        elif (bot_username and f"@{bot_username}" in text_lower) or "berry" in text_lower or "ဘယ်ရီ" in text_lower:
            is_mentioned = True
        else:
            return
    else:
    ...
    if is_mentioned:
        ...
        if not response:
            response = "မောင်‌‌ကအဲ့တာပြောဖို့ တို့ ကိုမှာမထားဘူး"
...
if __name__ == "__main__":
    TOKEN = "8784248371:AAGjQJ5YO_VQPxVSUYji2iz-Gfh8DMLg9TA"
